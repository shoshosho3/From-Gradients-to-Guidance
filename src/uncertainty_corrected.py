...
def compute_uncertainty_scores(model, unlabeled_loader):
    ...
    with torch.no_grad():
        for batch_idx, (x_batch, _) in enumerate(unlabeled_loader):
            ...
            combined_uncertainty = entropy - margin  # A common variant is entropy - margin

            # ### FIX ###: The index returned is relative to the unlabeled_loader.
            # This is the index within the 'unlabeled' list, not the original dataset.
            start_idx = batch_idx * unlabeled_loader.batch_size
            for i, score in enumerate(combined_uncertainty):
                pool_idx = start_idx + i
...
def run_uncertainty_active_learning_seed(seed):
    ...
    ### FIX ###: Initialize model and optimizer ONCE, before the loop starts.
    # This ensures the model learns continuously (fine-tuning) instead of
    # restarting from scratch ("cold start") in every round.
    ...
    for round_idx in range(num_rounds):
        ...
        # Training loop (now fine-tunes the existing model)
        model.train()
        ...
        if len(unlabeled) > 0:
            ...
            ### FIX ###: Correctly map the indices from the query pool back to the original dataset indices.
            # 1. Get the indices of the most uncertain samples *within the unlabeled pool*.
            query_pool_indices = [score_tuple[1] for score_tuple in uncertainty_scores[:selected_count]]
            # 2. Use these pool indices to get the actual dataset indices from the `unlabeled` list.
...
def run_random_active_learning_seed(seed):
    ...
    ### FIX ###: Initialize model and optimizer ONCE, before the loop starts.
    # This is critical for a fair comparison. The random baseline also benefits
    ...
    for round_idx in range(num_rounds):
        ...
        # Training loop (fine-tunes the existing model)
        model.train()
        for epoch in range(num_epochs):
            for x_batch, y_batch in train_loader:
                x_batch, y_batch = x_batch.to(device), y_batch.to(device)
...
def main():
    ...
    seeds = [42, 123]  # Using fewer seeds for a quicker test run
    ...
    if total_uncertainty_win > len(uncertainty_avg) * 0.7:
        ...
        print("   This suggests an issue with the uncertainty implementation or hyperparameters.")
    ...
    plt.title("Sanity Check: Uncertainty vs Random Active Learning (Corrected)")
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()

    plt.savefig('sanity_check_uncertainty_vs_random_CORRECTED.png', dpi=300, bbox_inches='tight')
    print(f"\nPlot saved as 'sanity_check_uncertainty_vs_random_CORRECTED.png'")
    plt.show()
    ...
    np.save('sanity_check_results_CORRECTED.npy', results_summary)
    print(f"Detailed results saved as 'sanity_check_results_CORRECTED.npy'")
